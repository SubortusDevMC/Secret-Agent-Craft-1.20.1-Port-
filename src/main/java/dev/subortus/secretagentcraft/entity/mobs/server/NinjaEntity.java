...
public class NinjaEntity extends Monster implements RangedAttackMob {
    public NinjaEntity(EntityType<? extends Monster> pEntityType, Level pLevel) {
    ...
    protected void dropCustomDeathLoot(DamageSource pSource, int pLooting, boolean pRecentlyHit) {
        ...
        if(!this.level().isClientSide()){
            int i = 0;
            do {
                this.spawnAtLocation(ModItems.THROWING_KNIFE.get().getDefaultInstance());
                i++;
            } while (i < random.nextInt(1, 3));
        }
    }
    ...
    public void shoot(LivingEntity pTarget){
        ThrowingKnifeEntity knife = new ThrowingKnifeEntity(this.level(), this);
        double d0 = pTarget.getEyeY() - (double)1.1F;
        double d1 = pTarget.getX() - this.getX();
        double d2 = d0 - knife.getY();
        double d3 = pTarget.getZ() - this.getZ();
        double d4 = Math.sqrt(d1 * d1 + d3 * d3) * (double)0.2F;
        knife.shoot(d1, d2 + d4, d3, 1.6F, 12.0F);
        knife.shooter(this);
    ...
    public ItemStack getMainHandItem() {
        return ModItems.THROWING_KNIFE.get().getDefaultInstance();
    }
}
