...
public class ArchAgentEntity extends Monster implements RangedAttackMob {
    public ArchAgentEntity(EntityType<? extends Monster> pEntityType, Level pLevel) {
    ...
    protected void dropCustomDeathLoot(DamageSource pSource, int pLooting, boolean pRecentlyHit) {
        ...
        if(!this.level().isClientSide()){
            int i = 0;
            do {
                this.spawnAtLocation(ModItems.GOLDEN_BULLET.get().getDefaultInstance());
                i++;
            } while (i < random.nextInt(1, 7));
            if(random.nextInt(3) == 0) {
                ItemStack item = new ItemStack(ModItems.GOLDEN_GUN.get());
    ...
    public void shoot(LivingEntity pTarget){
        ...
        bullet.setItem(ModItems.GOLDEN_BULLET.get().getDefaultInstance());
