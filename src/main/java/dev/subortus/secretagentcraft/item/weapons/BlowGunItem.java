...
public class BlowGunItem extends Item {
    ...
    public void onStopUsing(ItemStack stack, LivingEntity entity, int count) {
        ...
        if(flag || ((Player) entity).getInventory().hasAnyOf(Set.of(ModItems.POISON_DART.get()))){
            ...
            PoisonDartEntity dart = new PoisonDartEntity(level, entity);
            dart.setItem(ModItems.POISON_DART.get().getDefaultInstance());
            dart.shootFromRotation(entity, entity.getXRot(), entity.getYRot(), 0.0F, f * 4.0f, 1.0F);

            if(f == 1.0f){
                dart.isCritical(true);
            }

            if(stack.getEnchantmentLevel(Enchantments.POWER_ARROWS) > 0){
                dart.setPower(stack.getEnchantmentLevel(Enchantments.POWER_ARROWS));
            }
            ...
            dart.shooter(((Player) entity));
            stack.hurtAndBreak(1,entity, entityBreak -> entity.broadcastBreakEvent(entity.getUsedItemHand()));
            level.playSound(entity,entity.getOnPos(), SoundEvents.SKELETON_SHOOT, SoundSource.PLAYERS, 0.5f, 1.0f);
            ...
            if(!((Player) entity).getAbilities().instabuild){
                ((Player) entity).getInventory().getItem(((Player) entity).getInventory().findSlotMatchingItem(
                        ModItems.POISON_DART.get().getDefaultInstance())).shrink(1);
            }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if((pPlayer.getAbilities().instabuild || pPlayer.getInventory().hasAnyOf(Set.of(ModItems.POISON_DART.get())))){
