...
public class GoldenGunItem extends Item {
    public GoldenGunItem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public void onStopUsing(ItemStack stack, LivingEntity entity, int count) {
        ...
        if(flag || ((Player) entity).getInventory().hasAnyOf(Set.of(ModItems.GOLDEN_BULLET.get()))){
            ...
            BulletEntity gold_bullet = new BulletEntity(level, entity);
            gold_bullet.setItem(ModItems.GOLDEN_BULLET.get().getDefaultInstance());
            gold_bullet.shootFromRotation(entity, entity.getXRot(), entity.getYRot(), 0.0F, f * 4.0f, 1.0F);

            if(f == 1.0f){
                gold_bullet.isCritical(true);
            }

            if(stack.getEnchantmentLevel(Enchantments.POWER_ARROWS) > 0){
                gold_bullet.setPower(stack.getEnchantmentLevel(Enchantments.POWER_ARROWS));
            }

            if(stack.getEnchantmentLevel(Enchantments.FIRE_ASPECT) > 0){
                gold_bullet.isFire(true);
            }

            gold_bullet.shooter(entity);
            ...
            if(!((Player) entity).getAbilities().instabuild){
                ((Player) entity).getInventory().getItem(((Player) entity).getInventory().findSlotMatchingItem(
                        ModItems.GOLDEN_BULLET.get().getDefaultInstance())).shrink(1);
            }

            if(!level.isClientSide()){
                level.addFreshEntity(gold_bullet);
            }
        }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if((pPlayer.getAbilities().instabuild || pPlayer.getInventory().hasAnyOf(Set.of(ModItems.GOLDEN_BULLET.get())))){
